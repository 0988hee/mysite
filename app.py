...
# Flask class 생성
# 생성자 함수 필요한 인자는 파일의 이름
app = Flask(__name__)

# 네비게이터 -> 특정한 주소로 요청이 들어왔을 때 함수와 연결해준다
# route() 함수 안에 인자가 의미하는 것은?
# root url + 주소(route 함수에 인자)
@ app.route('/')
def index():
    # csv 파일을 로드
...
def dashboard():
    ...
    return render_template('index.html', columns = cols, values = value,
                           axis_x = x, axis_y = y)
